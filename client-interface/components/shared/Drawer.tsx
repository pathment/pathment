...
export function Drawer({ open, onClose, title, subtitle, width = 'md', footer, children }: DrawerProps) {
  ...
  return (
    <div className="fixed inset-0 h-screen h-[100dvh] z-50 flex justify-end overflow-hidden">
      <div
        className={`fixed inset-0 bg-black/40 dark:bg-black/70 transition-opacity duration-[250ms] ease-out ${shown ? 'opacity-100' : 'opacity-0'}`}
      ...
      <div
        ...
        className={`relative w-full ${WIDTHS[width]} h-full h-screen h-[100dvh] max-h-screen max-h-[100dvh] bg-card border-l border-slate-200 dark:border-slate-700 shadow-2xl dark:shadow-[-8px_0_30px_rgba(0,0,0,0.6)] flex flex-col outline-none transform-gpu transition-transform duration-[250ms] ease-[cubic-bezier(0.32,0.72,0,1)] ${shown ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className="flex items-start justify-between gap-3 px-6 py-4 border-b border-slate-200 dark:border-slate-700 shrink-0 bg-card z-10">
          ...
          <button
            onClick={() => onCloseRef.current()}
            aria-label="Close"
            className="p-1.5 text-slate-400 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 shrink-0 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {footer && (
          <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-700 flex justify-end gap-2 shrink-0 bg-card z-10">
            {footer}
          </div>
        )}
      </div>
