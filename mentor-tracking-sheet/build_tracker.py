...
rows = [
    ...
    ("", "Keep the header rows and the dropdown values as they are, and one mentee per row, and Pathment will be able to import this sheet straight in when it is ready. Email is the key that links a mentee across tabs. See import-format.md for the details. You do not need to do anything extra, just fill the sheet."),
    ("", ""),
    ("What the columns mean", ""),
    ("Email", "The mentee's email. The unique key that ties their rows together across tabs."),
...
for c in range(1, ncols + 1):
    ...
    cell.border = BORDER
center_cols = set(range(9, 24))
for rrow in range(2, nrows + 1):
    for c in range(1, ncols + 1):
        cell = t.cell(row=rrow, column=c)
        cell.font = BASE_FONT
        cell.border = BORDER
        cell.alignment = WRAP if c == ncols else (CENTER if c in center_cols else TOP)
...
def add_dv(sheet, col_letter, options, last):
    dv = DataValidation(type="list", formula1='"' + ",".join(options) + '"', allow_blank=True)
    dv.add(f"{col_letter}2:{col_letter}{last}")
    sheet.add_data_validation(dv)

add_dv(t, "L", ["Up", "Flat", "Down"], nrows)
add_dv(t, "M", ["Positive", "Neutral", "Low"], nrows)
add_dv(t, "N", ["On track", "Watch", "At risk"], nrows)
...
# dropdown of mentee names sourced from the tracker, reused on the log tabs
MENTEE_LIST = "'Mentee Tracker'!$A$2:$A$50"
...
dmen = DataValidation(type="list", formula1=MENTEE_LIST, allow_blank=True)
dmen.add(f"B2:B{dn}")
d.add_data_validation(dmen)
...
omen = DataValidation(type="list", formula1=MENTEE_LIST, allow_blank=True)
omen.add(f"B2:B{on}")
o.add_data_validation(omen)
